...
abstract class BaseClassTeacher extends BaseObject implements Persistent
{
    ...
    public function mayOperate($sOperation, $oUser = false) {
        $oUser = ClassTeacherPeer::getRightsUser($oUser);
        $bIsAllowed = false;
        if($oUser && ($this->isNew() || $this->getCreatedBy() === $oUser->getId()) && ClassTeacherPeer::mayOperateOnOwn($oUser, $this, $sOperation)) {
            $bIsAllowed = true;
        } else if(ClassTeacherPeer::mayOperateOn($oUser, $this, $sOperation)) {
            $bIsAllowed = true;
        }
        FilterModule::getFilters()->handleClassTeacherOperationCheck($sOperation, $this, $oUser, array(&$bIsAllowed));
        return $bIsAllowed;
    }

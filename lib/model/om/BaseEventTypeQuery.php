...
abstract class BaseEventTypeQuery extends ModelCriteria
{
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$obj = new EventType();
			$obj->hydrate($row);
			EventTypePeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
	...
	public function filterByUserRelatedByCreatedBy($user, $comparison = null)
	{
		if ($user instanceof User) {
			...
			return $this
				->addUsingAlias(EventTypePeer::CREATED_BY, $user->toKeyValue('PrimaryKey', 'Id'), $comparison);
		} else {
	...
	public function filterByUserRelatedByUpdatedBy($user, $comparison = null)
	{
		if ($user instanceof User) {
			...
			return $this
				->addUsingAlias(EventTypePeer::UPDATED_BY, $user->toKeyValue('PrimaryKey', 'Id'), $comparison);
		} else {

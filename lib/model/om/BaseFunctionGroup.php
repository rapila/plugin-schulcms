...
abstract class BaseFunctionGroup extends BaseObject implements Persistent
{
    ...
    public function setSchoolFunctions(PropelCollection $schoolFunctions, PropelPDO $con = null)
    {
        ...
        foreach ($schoolFunctionsToDelete as $schoolFunctionRemoved) {
            $schoolFunctionRemoved->setFunctionGroup(null);
        }
    ...
    public function removeSchoolFunction($schoolFunction)
    {
        if ($this->getSchoolFunctions()->contains($schoolFunction)) {
            ...
            $schoolFunction->setFunctionGroup(null);
        }
    ...
    public function mayOperate($sOperation, $oUser = false) {
        ...
        if($oUser && ($this->isNew() || $this->getCreatedBy() === $oUser->getId()) && FunctionGroupPeer::mayOperateOnOwn($oUser, $this, $sOperation)) {
            $bIsAllowed = true;
        } else if(FunctionGroupPeer::mayOperateOn($oUser, $this, $sOperation)) {
            $bIsAllowed = true;
        }
        FilterModule::getFilters()->handleFunctionGroupOperationCheck($sOperation, $this, $oUser, array(&$bIsAllowed));
        return $bIsAllowed;
    }

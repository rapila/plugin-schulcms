...
abstract class BaseNote extends BaseObject implements Persistent
{
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(NotePeer::ID)) {
            $modifiedColumns[':p' . $index++]  = '`id`';
        }
        if ($this->isColumnModified(NotePeer::NOTE_TYPE_ID)) {
            $modifiedColumns[':p' . $index++]  = '`note_type_id`';
        }
        if ($this->isColumnModified(NotePeer::BODY)) {
            $modifiedColumns[':p' . $index++]  = '`body`';
        }
        if ($this->isColumnModified(NotePeer::DATE_START)) {
            $modifiedColumns[':p' . $index++]  = '`date_start`';
        }
        if ($this->isColumnModified(NotePeer::DATE_END)) {
            $modifiedColumns[':p' . $index++]  = '`date_end`';
        }
        if ($this->isColumnModified(NotePeer::IS_INACTIVE)) {
            $modifiedColumns[':p' . $index++]  = '`is_inactive`';
        }
        if ($this->isColumnModified(NotePeer::IMAGE_ID)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case '`id`':
                        ...
                        break;
                    case '`note_type_id`':
                        $stmt->bindValue($identifier, $this->note_type_id, PDO::PARAM_INT);
                        break;
                    case '`body`':
     ...
     * @throws PropelException
     */
    public function getNoteType(PropelPDO $con = null, $doQuery = true)
    {
        if ($this->aNoteType === null && ($this->note_type_id !== null) && $doQuery) {
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep && !$this->alreadyInClearAllReferencesDeep) {
            ...
            if ($this->aNoteType instanceof Persistent) {
              $this->aNoteType->clearAllReferences($deep);
            }
    ...
    public function mayOperate($sOperation, $oUser = false) {
        ...
        if($oUser && ($this->isNew() || $this->getCreatedBy() === $oUser->getId()) && NotePeer::mayOperateOnOwn($oUser, $this, $sOperation)) {
            $bIsAllowed = true;
        } else if(NotePeer::mayOperateOn($oUser, $this, $sOperation)) {
            $bIsAllowed = true;
        }
        FilterModule::getFilters()->handleNoteOperationCheck($sOperation, $this, $oUser, array(&$bIsAllowed));
        return $bIsAllowed;
    }

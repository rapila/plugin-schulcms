...
abstract class BaseNoteType extends BaseObject implements Persistent
{
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(NoteTypePeer::ID)) {
            $modifiedColumns[':p' . $index++]  = '`id`';
        }
        if ($this->isColumnModified(NoteTypePeer::NAME)) {
        ...
        $sql = sprintf(
            'INSERT INTO `note_types` (%s) VALUES (%s)',
     ...
     * @return PropelObjectCollection|Note[] List of Note objects
     */
    public function getNotesJoinDocument($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
    {
        $query = NoteQuery::create(null, $criteria);
    ...
    public function mayOperate($sOperation, $oUser = false) {
        ...
        if($oUser && ($this->isNew() || $this->getCreatedBy() === $oUser->getId()) && NoteTypePeer::mayOperateOnOwn($oUser, $this, $sOperation)) {
            $bIsAllowed = true;
        } else if(NoteTypePeer::mayOperateOn($oUser, $this, $sOperation)) {
            $bIsAllowed = true;
        }
        FilterModule::getFilters()->handleNoteTypeOperationCheck($sOperation, $this, $oUser, array(&$bIsAllowed));
        return $bIsAllowed;
    }

...
abstract class BaseNoteTypeQuery extends ModelCriteria
{
     ...
     * @param   NoteTypeQuery|Criteria $criteria Optional Criteria to build the query from
     *
    ...
    public static function create($modelAlias = null, $criteria = null)
    {
        ...
        $query = new NoteTypeQuery(null, null, $modelAlias);
     ...
     *
     * @return   NoteType|NoteType[]|mixed the result, formatted by the current formatter
     */
    public function findPk($key, $con = null)
    {
        ...
        if ((null !== ($obj = NoteTypePeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
     ...
     *
     * @return                 NoteType A model object, or null if the key is not found
    ...
    {
        $sql = 'SELECT `id`, `name`, `created_at`, `updated_at`, `created_by`, `updated_by` FROM `note_types` WHERE `id` = :p0';
        try {
        ...
        if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
            $obj = new NoteType();
            $obj->hydrate($row);
            NoteTypePeer::addInstanceToPool($obj, (string) $key);
        }
        $stmt->closeCursor();
     ...
     * @param     PropelPDO $con an optional connection object
     *
     * @return PropelObjectCollection|NoteType[]|mixed the list of results, formatted by the current formatter
     */
    ...
    public function filterById($id = null, $comparison = null)
    {
        if (is_array($id)) {
            $useMinMax = false;
            if (isset($id['min'])) {
                $this->addUsingAlias(NoteTypePeer::ID, $id['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($id['max'])) {
                $this->addUsingAlias(NoteTypePeer::ID, $id['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;

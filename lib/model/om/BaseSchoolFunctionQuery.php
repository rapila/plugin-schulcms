...
abstract class BaseSchoolFunctionQuery extends ModelCriteria
{
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			$obj->hydrate($row);
			SchoolFunctionPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
	 ...
	 *
	 * @param     FunctionGroup|PropelCollection $functionGroup The related object(s) to use as filter
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *
	 * @return    SchoolFunctionQuery The current query, for fluid interface
	 */
	public function filterByFunctionGroup($functionGroup, $comparison = null)
	{
		...
		} elseif ($functionGroup instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this

...
abstract class BaseService extends BaseObject implements Persistent
{
    ...
    /**
     * @var        PropelObjectCollection|ServiceDocument[] Collection to store aggregation of ServiceDocument objects.
     */
    protected $collServiceDocuments;
    protected $collServiceDocumentsPartial;
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            if ($row[$startcol + 9] !== null) {
                $this->body_short = fopen('php://memory', 'r+');
                fwrite($this->body_short, $row[$startcol + 9]);
                rewind($this->body_short);
            } else {
    ...
    public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[9], $arr)) $this->setBodyShort($arr[$keys[9]]);
    ...
    {
        if ($this->aDocument === null && ($this->logo_id !== null) && $doQuery) {
     ...
     * @throws PropelException
     */
    public function getServiceCategory(PropelPDO $con = null, $doQuery = true)
    {
        if ($this->aServiceCategory === null && ($this->service_category_id !== null) && $doQuery) {
    ...
    public function clearServiceDocuments()
    {
        ...
        $this->collServiceDocumentsPartial = null;

        return $this;
    }

    /**
     * reset is the collServiceDocuments collection loaded partially
     *
     * @return void
     */
    public function resetPartialServiceDocuments($v = true)
    {
        $this->collServiceDocumentsPartial = $v;
    }
     ...
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param PropelPDO $con optional connection object
     * @return PropelObjectCollection|ServiceDocument[] List of ServiceDocument objects
     * @throws PropelException
     */
    public function getServiceDocuments($criteria = null, PropelPDO $con = null)
    {
        $partial = $this->collServiceDocumentsPartial && !$this->isNew();
        if (null === $this->collServiceDocuments || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collServiceDocuments) {
                ...
                $this->collServiceDocumentsPartial = false;
            }
        }
    ...
    /**
     * @param	ServiceDocument $serviceDocument The serviceDocument object to remove.
     * @return Service The current object (for fluent API support)
     */
    public function removeServiceDocument($serviceDocument)
    {
        if ($this->getServiceDocuments()->contains($serviceDocument)) {
            $this->collServiceDocuments->remove($this->collServiceDocuments->search($serviceDocument));
            if (null === $this->serviceDocumentsScheduledForDeletion) {
                $this->serviceDocumentsScheduledForDeletion = clone $this->collServiceDocuments;
                $this->serviceDocumentsScheduledForDeletion->clear();
            }
            $this->serviceDocumentsScheduledForDeletion[]= clone $serviceDocument;
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep && !$this->alreadyInClearAllReferencesDeep) {
            ...
            if ($this->aServiceCategory instanceof Persistent) {
              $this->aServiceCategory->clearAllReferences($deep);
            }
    ...
    public function mayOperate($sOperation, $oUser = false) {
        ...
        if($oUser && ($this->isNew() || $this->getCreatedBy() === $oUser->getId()) && ServicePeer::mayOperateOnOwn($oUser, $this, $sOperation)) {
            $bIsAllowed = true;
        } else if(ServicePeer::mayOperateOn($oUser, $this, $sOperation)) {
            $bIsAllowed = true;
        }
        FilterModule::getFilters()->handleServiceOperationCheck($sOperation, $this, $oUser, array(&$bIsAllowed));
        return $bIsAllowed;
    }

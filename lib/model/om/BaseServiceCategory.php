...
abstract class BaseServiceCategory extends BaseObject implements Persistent
{
    ...
    public function setServices(PropelCollection $services, PropelPDO $con = null)
    {
        ...
        foreach ($servicesToDelete as $serviceRemoved) {
            $serviceRemoved->setServiceCategory(null);
        }
    ...
    public function removeService($service)
    {
        if ($this->getServices()->contains($service)) {
            ...
            $this->servicesScheduledForDeletion[]= $service;
    ...
    public function mayOperate($sOperation, $oUser = false) {
        ...
        if($oUser && ($this->isNew() || $this->getCreatedBy() === $oUser->getId()) && ServiceCategoryPeer::mayOperateOnOwn($oUser, $this, $sOperation)) {
            $bIsAllowed = true;
        } else if(ServiceCategoryPeer::mayOperateOn($oUser, $this, $sOperation)) {
            $bIsAllowed = true;
        }
        FilterModule::getFilters()->handleServiceCategoryOperationCheck($sOperation, $this, $oUser, array(&$bIsAllowed));
        return $bIsAllowed;
    }

...
abstract class BaseServiceDocumentQuery extends ModelCriteria
{
     ...
     * @param   ServiceDocumentQuery|Criteria $criteria Optional Criteria to build the query from
     *
     ...
     * @param array $key Primary key to use for the query
                         A Primary key composition: [$service_id, $document_id]
     * @param     PropelPDO $con an optional connection object
     *
     * @return   ServiceDocument|ServiceDocument[]|mixed the result, formatted by the current formatter
     */
    ...
    protected function findPkSimple($key, $con)
    {
        ...
        if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            ServiceDocumentPeer::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
        }
        $stmt->closeCursor();
     ...
     * @param     PropelPDO $con an optional connection object
     *
     * @return PropelObjectCollection|ServiceDocument[]|mixed the list of results, formatted by the current formatter
     */
    ...
    public function filterByServiceId($serviceId = null, $comparison = null)
    {
        if (is_array($serviceId)) {
            ...
            if (isset($serviceId['min'])) {
                $this->addUsingAlias(ServiceDocumentPeer::SERVICE_ID, $serviceId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($serviceId['max'])) {
                $this->addUsingAlias(ServiceDocumentPeer::SERVICE_ID, $serviceId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
    ...
    public function filterByDocumentId($documentId = null, $comparison = null)
    {
        if (is_array($documentId)) {
            ...
            if (isset($documentId['min'])) {
                $this->addUsingAlias(ServiceDocumentPeer::DOCUMENT_ID, $documentId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($documentId['max'])) {
                $this->addUsingAlias(ServiceDocumentPeer::DOCUMENT_ID, $documentId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;

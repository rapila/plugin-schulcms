...
abstract class BaseServiceQuery extends ModelCriteria
{
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$obj = new Service();
			$obj->hydrate($row);
			ServicePeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
	 ...
	 *
	 * @param     ServiceCategory|PropelCollection $serviceCategory The related object(s) to use as filter
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *
	 * @return    ServiceQuery The current query, for fluid interface
	 */
	public function filterByServiceCategory($serviceCategory, $comparison = null)
	{
		...
		} elseif ($serviceCategory instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this
	...
	public function filterByServiceDocument($serviceDocument, $comparison = null)
	{
		...
		} elseif ($serviceDocument instanceof PropelCollection) {
			return $this

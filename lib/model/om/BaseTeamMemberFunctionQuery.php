...
abstract class BaseTeamMemberFunctionQuery extends ModelCriteria
{
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			$obj->hydrate($row);
			TeamMemberFunctionPeer::addInstanceToPool($obj, serialize(array((string) $row[0], (string) $row[1])));
		}
		$stmt->closeCursor();
	 ...
	 *
	 * @param     SchoolFunction|PropelCollection $schoolFunction The related object(s) to use as filter
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *
	 * @return    TeamMemberFunctionQuery The current query, for fluid interface
	 */

...
class MyServicesWidgetModule extends PersistentWidgetModule {
	...
	public function listServices($aSettings) {
		...
		$oServicesPage = PageQuery::create()->filterByIdentifier(SchoolPeer::PAGE_IDENTIFIER_SERVICES)->findOne();
		...
		foreach($oQuery->find() as $oServiceMember) {
			...
			$aServiceInfo['ServiceLink'] = LinkUtil::link($oService->getServiceLink($oServicesPage), 'FrontendManager');
